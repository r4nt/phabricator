...
final class DifferentialInlineCommentMailView
  ...
  public function buildMailSection() {
    ...
    $all_comments = $this->loadAllComments($inlines);
	...
	$nested_comments = null;
        if ($show_email_context) {
          $nested_comments = $this->nestCommentHistory($comment,
            $comments_by_line_number);
        }
        ...
        if (!$show_email_context) {
          ...
          $section->addPlaintextFragment(pht('Comment at: %s:%s', $file, $range));
          $section->addPlaintextFragment($patch_text);
          $section->addPlaintextFragment('----------------');
          $section->addPlaintextFragment($nested_comments);
          $section->addPlaintextFragment($spacer_text);
        }
  ...
  private function nestCommentHistory(
    ...
    foreach ($previous_comments as $previous_comment) {
      ...
      if ($user) {
        array_unshift($nested, pht('%s wrote:', $user->getName()));
      }
    }
